...
def write_daily_journal():
    ...
    content, _ = call_deepseek(system_prompt, max_tokens=config.DEEPSEEK_MAX_TOKENS)
    state.record_call()
    if content:
...
def generate_reply_stream(context, app_name=None, use_cache=True, session_id=None):
    ...
    print("[TRACE 1] after record_prompt")
    state.record_call()
    print("[TRACE 2] after record_call")
    
    full_reasoning = ""
    raw_reasoning = ""
    full_content = ""
    
    print("[TRACE 3] before call_deepseek_stream")
    gen = call_deepseek_stream(system_prompt, max_tokens=config.DEEPSEEK_MAX_TOKENS, session_id=target_session, source="watch")
    print("[TRACE 4] generator created")
    
    print("[TRACE 5] before entering for loop")
    try:
        for event_type, data in gen:
            print(f"[TRACE 6] inside for loop, event: {event_type}")
