...
@router.post("/upload")
async def upload_image(image: UploadFile = File(...), caption: str = Form(None)):
    """
    接收图片上传,将图片内容作为消息发送给Lin。
    """
    from app import generate_reply, send_to_bark
    import base64
    ...
    try:
        ...
        # 构造消息内容
        if caption:
            context = f"Anna发了一张图片,并说: {caption}"
        else:
            context = "Anna发了一张图片"
        
        # 添加到对话历史
        state.add_conversation_turn("anna", context, image_data=image_b64)
