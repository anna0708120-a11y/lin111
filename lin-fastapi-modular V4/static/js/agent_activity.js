...
(function () {
  ...
  class Turn {
    ...
    thinking(text) { if (!text) return; if (!this.currentText) this.createTextSegment(); const row = this.currentText.closest('.msg'); let box = row && row.querySelector('.lin-agent-thinking'); if (!box && row) { box = document.createElement('div'); box.className = 'think-box lin-agent-thinking'; row.insertBefore(box, row.querySelector('.msg-row')); } if (box) box.textContent += text; this._scroll(); }
