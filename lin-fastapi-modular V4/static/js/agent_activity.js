...
(function () {
  ...
  class Turn {
    ...
    thinking(text) { if (!text) return; if (!this.currentText) this.createTextSegment(); let box = this.currentText.parentElement.querySelector('.lin-agent-thinking'); if (!box) { box = document.createElement('div'); box.className = 'think-box lin-agent-thinking'; this.currentText.parentElement.insertBefore(box, this.currentText); } box.textContent += text; this._scroll(); }
    handleTimelineEvent(raw) { const event = unwrap(raw); if (!event || event.type === 'memory') return; const status = String(event.status || '').toLowerCase(); const id = String(event.id || ''); const terminal = ['success', 'failed', 'skipped', 'not_executed', 'unknown'].includes(status); const failed = ['failed', 'unknown'].includes(status); const mapped = { ...event, type: terminal ? 'tool.complete' : this.activity?.byTool?.has(id) ? 'tool.progress' : 'tool.start', tool_id: id, name: event.display_name || event.name || event.type, summary: event.summary || event.reason, is_error: failed || undefined }; this.handleEvent(mapped); }
