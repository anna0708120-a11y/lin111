...
(function () {
  ...
  class Turn {
    ...
    handleTimelineEvent(raw) { const event = unwrap(raw); if (!event || event.type === 'memory') return; const status = String(event.status || '').toLowerCase(); const id = String(event.id || ''); const mapped = { ...event, type: status === 'success' || status === 'failed' ? 'tool.complete' : this.activity?.byTool?.has(id) ? 'tool.progress' : 'tool.start', tool_id: id, name: event.display_name || event.name || event.type, summary: event.summary || event.reason }; this.handleEvent(mapped); }
